...
func (*FeedItem) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case feeditem.FieldTitle, feeditem.FieldDescription, feeditem.FieldLink:
			values[i] = new(sql.NullString)
		case feeditem.FieldPublishedAt, feeditem.FieldCreatedAt, feeditem.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		case feeditem.ForeignKeys[0]: // feed_id
	...
	for i := range columns {
		...
		case feeditem.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullScanner); !ok {
				...
				fi.feed_id = new(uuid.UUID)
				*fi.feed_id = *value.S.(*uuid.UUID)
			}
		default:
...
func (fi *FeedItem) String() string {
	...
	builder.WriteString(fi.PublishedAt.Format(time.ANSIC))
	builder.WriteString(", ")
