...
// QueryFeedItems chains the current query on the "feed_items" edge.
func (fq *FeedQuery) QueryFeedItems() *FeedItemQuery {
...
func (fq *FeedQuery) Clone() *FeedQuery {
	...
	return &FeedQuery{
		...
		withFeedItems: fq.withFeedItems.Clone(),
...
// WithFeedItems tells the query-builder to eager-load the nodes that are connected to
// the "feed_items" edge. The optional arguments are used to configure the query builder of the edge.
func (fq *FeedQuery) WithFeedItems(opts ...func(*FeedItemQuery)) *FeedQuery {
	query := (&FeedItemClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withFeedItems = query
	return fq
}
...
func (fq *FeedQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Feed, error) {
	...
	if query := fq.withFeedItems; query != nil {
		if err := fq.loadFeedItems(ctx, query, nodes,
			func(n *Feed) { n.Edges.FeedItems = []*FeedItem{} },
			func(n *Feed, e *FeedItem) { n.Edges.FeedItems = append(n.Edges.FeedItems, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (fq *FeedQuery) loadFeedItems(ctx context.Context, query *FeedItemQuery, nodes []*Feed, init func(*Feed), assign func(*Feed, *FeedItem)) error {
	...
	query.Where(predicate.FeedItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(feed.FeedItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.feed_id
		if fk == nil {
			return fmt.Errorf(`foreign-key "feed_id" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
